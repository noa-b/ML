...
const featureFuncs = require("../common/featureFuncs.js");
...
// This extra section helps to install the package 
if(fs.existsSync(constants.DATASET_DIR)){
    fs.readdirSync(constants.DATASET_DIR).forEach(fileName => 
        fs.rmSync(constants.DATASET_DIR + 
            "/" + fileName, {recursive:true})
        );
...
const {session, partic: partic, drawings}=JSON.parse(content);
